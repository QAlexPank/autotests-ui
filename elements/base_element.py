...
class BaseElement:
    ...
    def get_raw_locator(self, nth: int = 0, **kwargs) -> str:
        # Возвращается строковое XPath-представление локатора для покрытия.
        # Важно: Playwright сам не даёт доступ к исходному локатору, поэтому нужно формировать вручную.
        # Используется XPath, так как он легко поддерживает индексацию ([n+1]).
        return f"//*[@data-testid='{self.locator.format(**kwargs)}'][{nth + 1}]"

    def track_coverage(self, action_type: ActionType, nth: int = 0, **kwargs):
        # Трекается действие над элементом, отправляя данные в coverage-трекер.
        # Передаётся:
        # - XPath-селектор
    ...
    def click(self, nth: int = 0, **kwargs):
        ...
        # После успешного клика трекается действие как CLICK
    ...
    def check_visible(self, nth: int = 0, **kwargs):
        ...
        # Трекается видимость как VISIBLE
    ...
    def check_have_text(self, text: str, nth: int = 0, **kwargs):
        ...
        # Трекается наличие текста как TEXT
